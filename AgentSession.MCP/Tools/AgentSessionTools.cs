...
public sealed class AgentSessionTools
{
    ...
    public async Task<SaveFinalPlanResponse> save_final_plan(SaveFinalPlanRequest request)
    {
        try
        {
            return await _service.SaveFinalPlanAsync(request, CancellationToken.None);
        }
        catch (ValidationException ex)
        {
            throw new InvalidOperationException($"Invalid request: {ex.Message}", ex);
    ...
    public async Task<GetLatestFinalPlanResponse> get_latest_final_plan(GetLatestFinalPlanRequest request)
    {
        try
        {
            return await _service.GetLatestFinalPlanAsync(request, CancellationToken.None);
        }
        catch (ValidationException ex)
        {
            throw new InvalidOperationException($"Invalid request: {ex.Message}", ex);
